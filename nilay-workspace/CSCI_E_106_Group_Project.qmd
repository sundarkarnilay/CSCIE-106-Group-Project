...
for(name in categoricalVarsColumnNames.df){
  c <- unique(ames.train_data.df[name])
  levels <- c[,1]
  labels <- c()
  for (i in 1:nrow(c)) {
    labels <- append(labels, i)
  }
  ames.train_data.df[,name] <- factor(ames.train_data.df[,name],
                       levels = levels,
                       labels = labels)
}
```
...
#library(Hmisc)
#ccs <- as.matrix(ames.train_data.df)
#matrix <- rcorr(ccs, type="pearson")
#library(corrr)
#corrr::correlate(ames.train_data.df, method = "pearson")
```
