...
metrics <- data.frame(
  Model = c("Full Model", "Null Model", "Both Directions (AIC)", "Both Directions (BIC)", "Both Directions (p-value)"),
  AIC = c(AIC(fm), AIC(nm), AIC(sm_both_aic), AIC(sm_both_bic), AIC(sm_both_p)),
  BIC = c(BIC(fm), BIC(nm), BIC(sm_both_aic), BIC(sm_both_bic), BIC(sm_both_p)),
  R_Squared = c(summary(fm)$r.squared, summary(nm)$r.squared, 
                summary(sm_both_aic)$r.squared, summary(sm_both_bic)$r.squared, 
                summary(sm_both_p)$r.squared),
  Adj_R_Squared = c(summary(fm)$adj.r.squared, summary(nm)$adj.r.squared, 
                    summary(sm_both_aic)$adj.r.squared, summary(sm_both_bic)$adj.r.squared, 
                    summary(sm_both_p)$adj.r.squared),
  Num_Predictors = c(length(coefficients(fm)), length(coefficients(nm)),
                     length(coefficients(sm_both_aic)), length(coefficients(sm_both_bic)),
                     length(coefficients(sm_both_p)))
)
...
These are on the high end of VIFs. None are greater than 10. 
remedial methods are not necessary.
BsmtFin.SF.1: 7.887440
Bsmt.Unf.SF: 7.267218
Bldg.Type: 7.749723
X1st.Flr.SF: 6.223949
Exter.Qual: 6.279509
```{r}
...
```{r, echo=TRUE}
transformed_model2 = lm(price^(lambda) ~ MS.Zoning + Lot.Area + Land.Contour + Lot.Config + 
    Land.Slope + Neighborhood + Condition.1 + Bldg.Type + Overall.Qual + 
    Overall.Cond + Year.Built + Year.Remod.Add + Roof.Matl + 
    Mas.Vnr.Type + Mas.Vnr.Area + Exter.Qual + Bsmt.Qual + Bsmt.Exposure + 
    BsmtFin.SF.1 + BsmtFin.Type.2 + BsmtFin.SF.2 + Bsmt.Unf.SF + 
...
y_hat_transformed_model2 <- predict(transformed_model2, newdata = ames.test_data.df)
Model.transformed_model2 <- data.frame(obs = ames.test_data.df$price, pred = y_hat_transformed_model2)
transformed_model_summary2 <- defaultSummary(Model.transformed_model2)

# Display the summary
defaultSummary(transformed_model_summary2)
```

```{r, echo=TRUE}
print(y_hat_transformed_model2)
```

```{r, echo=TRUE}
out <- rbind(defaultSummary(transformed_model),defaultSummary(transformed_model2),defaultSummary(ModelTest2.2))
dimnames(out)[[1]] <- c("Best Linear (Train)","Best Linear (Test)","Tree Model (Train)","Tree Model (Test)")
out
```
