...
# Apply normalization function to each numeric column of ames_data.df
q1_norm <- as.data.frame(lapply(ames_data.df, function(x) if(is.numeric(x)) normalize(x) else x))

# Apply normalization to training and test datasets
q1_norm_train <- as.data.frame(lapply(ames.train_data.df, function(x) if(is.numeric(x)) normalize(x) else x))
q1_norm_test <- as.data.frame(lapply(ames.test_data.df, function(x) if(is.numeric(x)) normalize(x) else x))

# Convert non-numeric variables to numeric
q1_norm_train <- lapply(q1_norm_train, as.numeric)
q1_norm_test <- lapply(q1_norm_test, as.numeric)

# Handle missing values (if any)
q1_norm_train <- na.omit(q1_norm_train)
q1_norm_test <- na.omit(q1_norm_test)

# Train neural network
q1_nn <- neuralnet(price ~ ., q1_norm_train, hidden=c(6,4))

# Plot neural network
plot(q1_nn)
```
```{r,echo=TRUE}
print(str(q1_norm_train[-1]))
```

```{r, echo=TRUE}
set.seed(567)
model_results <- compute(q1_nn, q1_norm_train[-1])
